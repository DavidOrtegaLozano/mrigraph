...
def draw_graph(G):
    ...
    G.graph["show_text"] = visual_params["show_text"]
    G.graph["node_size"] = visual_params["node_size"]
    ...
    if len(nodes) > 0 and len(precomputed_pos) == len(nodes):
        ...
        nx.set_node_attributes(G, clean_pos, "pos")
    else:
    ...
    fig = go.Figure(
        data=(edge_trace + [node_trace,  eweights_trace_hover, eweights_trace_markers]),
        layout=go.Layout(
            showlegend=False,
